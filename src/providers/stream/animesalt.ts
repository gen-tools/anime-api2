...
async function resolvePageSources(html: string, pageUrl: string, base: string): Promise<SourceResult[]> {
  ...
  const isIndianLang =
    /category\/language\/(?:hindi|tamil|telugu|bengali|malayalam|kannada)/i.test(html) ||
    /languageToCode/i.test(html) ||
    /\bhindi\b/i.test(html) ||
    /as-cdn\d+\.top/i.test(html);
  const detectedLang = isIndianLang
    ? { audioLanguage: 'hi', language: 'Multi Audio' }
    : { audioLanguage: 'ja', language: 'Japanese' };
  ...
  if (ajaxResult) {
    ...
    return initial.filter(s => !s.url.includes('admin-ajax.php|nonce=')).map(s => ({ ...detectedLang, ...s }));
  }

  return initial.map(s => ({ ...detectedLang, ...s }));
}
